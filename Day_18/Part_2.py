...
def different_calculation(expression):
    ...
    while True:
        if "+" in expressionArr:
            index = expressionArr.index("+")
            resultArr = []
            for i in range(index-1):
                resultArr.append(expressionArr[i])
            resultArr.append(str(eval(expressionArr[index-1] + expressionArr[index] + expressionArr[index+1])))
            for i in range(index+2, len(expressionArr)):
                resultArr.append(expressionArr[i])
            expressionArr = copy.deepcopy(resultArr)
        elif "*" in expressionArr:
            last_part = []
